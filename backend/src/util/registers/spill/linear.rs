use crate::util::registers::context::linear_spill;
...
pub fn spill(
    ...
    resulting.extend(initial_iter.map(|s| replace::statement(s, &spill.var, &replacement)));
    ...
    for block in start_block.block_iter().skip(1) {
        ...
        block.set_statements(
            tmp.into_iter()
                .map(|s| replace::statement(s, &spill.var, &replacement))
                .collect(),
        );
    }

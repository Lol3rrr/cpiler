    ...
    pub fn replacement(&self) -> ir::Variable {
        match self {
            Self::Linear(res) => res
                .var
                .next_gen()
                .set_description("Linear spilled Variable"),
            Self::Conditional(cond) => match cond {
                conditional_spill::SpillResult::OuterVariable { var } => var
                    .next_gen()
                    .set_description("Outer Conditional spilled Variable"),
                conditional_spill::SpillResult::InnerVariable { var, .. } => var
                    .next_gen()
                    .set_description("Inner Conditional spilled Variable"),
            },
            Self::Loop(cond) => match cond {
                loop_spill::SpillResult::Outer { var } => var
                    .next_gen()
                    .set_description("Outer Loop spilled Variable"),
            },
        }
    }
}
    ...
    pub fn determine_spill_var(
        ...
        match self {
            Self::Linear { start, end } => {
                let res = linear_spill::determine_spill_var(
                    largest_vars,
                    spill_block,
                    start_index,
                    end.clone(),
